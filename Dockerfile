...
COPY --from=build /app/build/libs/*.jar /app/customer-service.jar
...
ENTRYPOINT [ "sh", "-c", "echo 'Waiting for discovery-service to start'; \
  ...
  echo 'discovery-service is up and running customer-service'; \
  exec java -jar /app/customer-service.jar" ]